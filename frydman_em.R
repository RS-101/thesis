...
em_estimate <- function(
  ...
  c_k = NULL,                         # constants for i > I in (24); optional
  # Control
  ...
  for (iter in seq_len(max_iter)) {
    ...
    if (is.finite(dz) && is.finite(dl) && max(dz, dl) < tol) { conv <- TRUE; break }
  }
...
res_em <- em_estimate(z_init = z_i, lambda_init = lambda_n, Q_full, s_j_full, Q_i, full_A_m, A_m, A_u, A_c, T_star, E_star, t_m, t_u, t_c, N_star,d_n, c_k, verbose = T)
...
estimators <- calc_F_and_hazards(z_head = res_em$z_head,
                   r = Q_i[,2], 
                   z_tail = res_em$z_tail, 
                   Q = Q_full[[2]], 
                   s_eval = seq(0,10, by = 0.5), 
                   res_em$lambda, 
                   tstar = T_star)
...
length(estimators$s_eval)
length(estimators$F12)


df <- data.frame(F12 = estimators$F12, s = estimators$s_eval)
df %>% arrange(s)
